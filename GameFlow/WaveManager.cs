...
public class WaveDefinition
{
    ...
    [Header("Spawns Al�atoires (En continu)")]
    public List<EnemySpawnConfig> randomSpawns;

    [Header("Spawns Fixes (�v�nements)")]
...
public class WaveManager : MonoBehaviour
{
    ...
    [Header("�tat (Read Only)")]
    public int currentWaveIndex = 0;
    public float waveTimer = 0f;

    [Header("S�curit� Spawn")] // --- NOUVEAU ---
    [SerializeField] private LayerMask invalidSpawnLayers; // Mettre Obstacle + Destructible (POI)
    [SerializeField] private float checkRadius = 0.5f;     // Taille de la zone � v�rifier
    ...
    private void Update()
    {
        ...
        // 2. V�rification des Spawns Fixes (Boss/Elites)
        CheckTimedSpawns();

        // 3. V�rification des Spawns Al�atoires (Horde)
        ...
        if (_spawnTimer >= currentWave.spawnInterval)
        {
            ...
            else if (EnemyManager.Instance != null)
            {
                ...
                {
                    canSpawn = true; // Une place s'est lib�r�e !
                }
            }
    ...
    private void InitializeWave(int index)
    {
        ...
        Debug.Log($"WaveManager: D�but {wave.waveName}");
    }
    ...
    private void NextWave()
    {
        ...
        if (nextIndex >= waves.Count)
        {
            if (loopLastWave && waves.Count > 0)
            {
                ...
                Debug.Log("WaveManager: Boucle sur la derni�re vague");
            }
            else
            {
                Debug.Log("WaveManager: Toutes les vagues sont termin�es !");
                enabled = false;
            }
        }
        else
    ...
    private bool TryGetValidSpawnPosition(out Vector3 position)
    {
        ...
        for (int i = 0; i < maxAttempts; i++)
        {
            ...
            // V�rification Physics.CheckSphere
            // On v�rifie si �a touche le Layer "Obstacle" ou "Destructible" (POI)
        ...
        return false; // Pas trouv� de place (trop dense)
    }
}